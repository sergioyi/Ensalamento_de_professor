...
function adicionar() {
  ...
  if (localStorage.valores_horario) {
    vetor = JSON.parse(localStorage.getItem('valores_horario')); 
  }
  let novoItem = (inserir.value);
  vetor.push(novoItem);
  inserir.value = "";
  localStorage.valores_horario = JSON.stringify(vetor);
...
function mostrar(){
    if(localStorage.valores_horario){//se tiver "valores" nolocalStorage execute o de baixo
        vetor = JSON.parse(localStorage.getItem('valores_horario'));//está pegando o "valores" que tem os valores do "vetor" e devolve a ela
    }
    for(var i in vetor){
