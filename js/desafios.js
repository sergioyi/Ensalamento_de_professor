...
function adicionar() {
  ...
  if (localStorage.valores_desafio) {
    vetor = JSON.parse(localStorage.getItem('valores_desafio')); 
  }
  let novoItem = (inserir.value);
  vetor.push(novoItem);
  inserir.value = "";
  localStorage.valores_desafio = JSON.stringify(vetor);
...
function mostrar(){
    if(localStorage.valores_desafio){//se tiver "valores" nolocalStorage execute o de baixo
        vetor = JSON.parse(localStorage.getItem('valores_desafio'));//está pegando o "valores" que tem os valores do "vetor" e devolve a ela
    }
    for(var i in vetor){
