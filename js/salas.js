...
function adicionar() {
  ...
  if (localStorage.valores_salas) {
    vetor = JSON.parse(localStorage.getItem('valores_salas')); 
  }
  let novoItem = (inserir.value);
  vetor.push(novoItem);
  inserir.value = "";
  localStorage.valores_salas = JSON.stringify(vetor);
...
function mostrar(){
    if(localStorage.valores_salas){//se tiver "valores" nolocalStorage execute o de baixo
        vetor = JSON.parse(localStorage.getItem('valores_salas'));//está pegando o "valores" que tem os valores do "vetor" e devolve a ela
    }
    for(var i in vetor){
