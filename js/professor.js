...
function adicionar() {
  ...
  if (localStorage.valores_professor) {
    vetor = JSON.parse(localStorage.getItem('valores_professor')); 
  }
  let novoItem = (inserir.value);
  vetor.push(novoItem);
  inserir.value = "";
  localStorage.valores_professor = JSON.stringify(vetor);
...
function mostrar(){
    if(localStorage.valores_professor){//se tiver "valores" nolocalStorage execute o de baixo
        vetor = JSON.parse(localStorage.getItem('valores_professor'));//está pegando o "valores" que tem os valores do "vetor" e devolve a ela
    }
    for(var i in vetor){
