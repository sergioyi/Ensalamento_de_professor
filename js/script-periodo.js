...
const alterarLine = (evt) => {
    let Periodo = prompt('Escreva o Periodo: ');
...
function aoIncluir(evt) {
    const objPeriodo = document.querySelector('#Periodo input');
    ...
    // Atualiza o localStorage com a lista atualizada
    atualizarLocalStorage();
    objTableBody.appendChild(createNewLine(objPeriodo.value));
}
...
const init = () => {
    ...
    listaPeriodo.forEach(item => {
        const objTableBody = document.querySelector('.tabela tbody');
        objTableBody.appendChild(createNewLine(item.Periodo));
    });
};
