...
function adicionar() {
  ...
  if (localStorage.valores_periodo) {
    vetor = JSON.parse(localStorage.getItem('valores_periodo')); 
  }
  let novoItem = (inserir.value);
  vetor.push(novoItem);
  inserir.value = "";
  localStorage.valores_periodo = JSON.stringify(vetor);
...
function mostrar(){
    if(localStorage.valores_periodo){//se tiver "valores" nolocalStorage execute o de baixo
        vetor = JSON.parse(localStorage.getItem('valores_periodo'));//está pegando o "valores" que tem os valores do "vetor" e devolve a ela
    }
    for(var i in vetor){
